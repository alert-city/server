...
import { ActivationResponseDto, UserResponseDto } from '@/modules/user/dtos/user-response.dto';
import { UpdateUserRequestDto, UserRequestDto, VerificationInfoRequestDto } from '@/modules/user/dtos/user-request.dto';
...
export class UserService {
  ...
  async createUser(input: UserRequestDto): Promise<UserResponseDto> {
    ...
    const result = await this.notificationService.sendActivationEmail(newUser);
  ...
  async resendActivationEmail(username: string): Promise<boolean> {
    ...
    return await this.notificationService.sendActivationEmail(foundUser);
  }
}